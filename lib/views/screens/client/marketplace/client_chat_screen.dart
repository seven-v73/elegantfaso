...
class _ClientChatScreenState extends State<ClientChatScreen> {
  ...
  Future<void> _scrollToBottom({bool animated = true}) async {
    // Ne rien faire si le contrôleur n'est pas prêt ou s'il n'y a pas de messages
    ...
    try {
      ...
      // Optionnel : surligner brièvement l'élément ciblé
      _scrollController.highlight(lastIndex);
    } catch (e) {
      debugPrint('Erreur lors du scroll vers le bas : $e');
    }

    // Mise à jour de l'état si le widget est toujours monté
    if (mounted) {
      setState(() {
  ...
  void _handleCameraSelection() {
    // Implémentez la capture d'image
    Navigator.pop(context);
  }

  void _handleDocumentSelection() {
    // Implémentez la sélection de document
    Navigator.pop(context);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: _showScrollToBottom
        ...
        child: Stack(
          children: [
            Icon(Icons.arrow_downward, color: onPrimaryColor),
            if (_hasNewMessages)
              Positioned(
                top: 0,
                right: 0,
                        ...
                        children: [
                          if (!isMe)
                            Padding(
                              ...
                              child: Text(
                                data['senderName']?.toString() ?? 'Boutique',
                                style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  fontSize: 12,
                                  color: isMe
